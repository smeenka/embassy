...
impl PwmTest {
    ...
    async fn task(&mut self) {
        loop {
            self.duty = (self.duty + 20) % self.max3;
...
async fn main(spawner: Spawner) {
    ...
    let d4_pb5 = PwmPin::new(p.PA7, OutputType::PushPull);
    ...
    spawner.spawn(unwrap!(pwm_task(pwm_test)));
    unsafe { cortex_m::peripheral::NVIC::unmask(interrupt::TIM3) };
